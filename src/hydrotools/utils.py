...
def hypsometric_comparison(d1: str, d2: str, dem: str, d1_label: str, d2_label: str, dif: bool = False):
    from raster import from_raster
    import matplotlib.pyplot as plt

    dem_data = from_raster(dem)

    d1_data = from_raster(d1)
    d2_data = from_raster(d2)

    stream_mask = da.compute(~da.ma.getmaskarray(d1_data))

    elev = np.round(dem_data[stream_mask].compute()).astype("int64")

    def mean_over_bands(bands, data):
        labels = bands - bands.min()

        srt = np.argsort(labels)

        sums = np.bincount(labels[srt], data[srt])

        indices, counts = np.unique(labels, return_counts=True)

        avg = (sums[indices] / counts)

        return avg, indices + bands.min()

    avg_d1, elev_bands = mean_over_bands(elev, d1_data[stream_mask].compute())
    avg_d2, _ = mean_over_bands(elev, d2_data[stream_mask].compute())

    plt.gca().invert_xaxis()

    if dif:
        plt.plot(elev_bands, avg_d1 - avg_d2, label=f"{d1_label} - {d2_label}")
    else:
        plt.plot(elev_bands, avg_d1, label=d1_label)
        plt.plot(elev_bands, avg_d2, label=d2_label)
